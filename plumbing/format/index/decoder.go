...
var (
	...
	// ErrUnknownMandatoryExtension is returned when an index extension is encountered that is considered mandatory
	ErrUnknownMandatoryExtension = errors.New("unknown mandatory extension")
)
...
func (d *Decoder) readExtension(idx *Index) error {
	...
	default:
		if header[0] < 'A' || header[0] > 'Z' {
			return ErrUnknownMandatoryExtension
		}
